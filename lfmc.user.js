...
// @resource yuiskin http://yui.yahooapis.com/combo?3.14.1/widget-base/assets/skins/sam/widget-base.css&3.14.1/tabview/assets/skins/sam/tabview.css
// ==/UserScript==
...
YUI().use('node', 'tabview', 'querystring', 'pjax', function(Y) {
	...
	initPlayer: function() {
	    ...
	    morelike.all('.compact_info_snb').each(function(item) {
		item.remove(false);
		if (item.hasClass('first')) {
		    item.removeClass('first');
		}
